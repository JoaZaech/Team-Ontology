...
The initial persistence backend is an embedded SQLite graph store. It keeps
typed nodes, relationships, aggregate views, policy bindings, and scenario
checkpoints in one offline file while preserving the JSON exports as portable
review artifacts. It is selected for deterministic local builds, easy backup,
and zero network-service dependency; it is not a production graph-service
decision.

Any future production backend must be evaluated against query depth, latency,
volume, consistency, temporal requirements, audit, backup, recovery,
operational complexity, portability, and vendor lock-in.
...
## 21. Test-dataset storage, precomputation, and policy-binding plan

### Scope and boundary

This phase stores the supplied test dataset as a reproducible graph and
materializes the evidence required to evaluate the documented scenario tasks
and goals. It does not add an API, frontend adapter, live wallet-control
integration, or a connection to `live_layer`. All outputs are offline files
that can be inspected, regenerated, and used by the existing deterministic
simulation.

### Storage model

Store one immutable dataset release under a versioned directory. Preserve the
eleven CSV files and their schemas as the source of record; never rewrite raw
source rows. Build the following derived artifacts from that release:

```text
data release
  -> validated source records
  -> canonical source graph snapshot
  -> temporal evidence index
  -> materialized aggregate views
  -> policy-bound scenario evaluation fixtures
  -> manifest with hashes and versions
```

The canonical graph remains a portable node-link JSON document. Each node and
relationship must retain its stable graph ID, source file, source row ID,
source fields, event time when present, source-as-of time, and graph version.
Derived records additionally retain `snapshot_id`, `calculation_version`,
`as_of`, and `supporting_event_ids`. This makes every stored fact traceable to
the test dataset and prevents a regenerated result from silently mixing data
releases or algorithms.

Use these artifact names within each build directory:

```text
source_graph.json                 complete typed graph for all 11 files
historical_evidence_index.json    card-partitioned, time-ordered source events
precomputed_evidence.json         end-of-history aggregate inspection view
scenario_checkpoints.json         as-of materializations for each test attempt
policy_bindings.json              confirmed/simulation policies and graph links
dataset_manifest.json             input hashes, row counts, and artifact versions
```

`source_graph.json` is the only graph of raw test records. Aggregate views are
separate derived artifacts, rather than replacing source nodes or mutating raw
relationships. Historical `TR...` and scenario `AU...` event namespaces remain
separate in every artifact.

### Precompute priorities

Materialize the evidence needed by the scenario goals first. Every aggregate
uses only records strictly earlier than its `as_of` timestamp, and every value
lists its supporting authorization IDs.

1. Build the complete source graph and validate that every source row is
  represented, every edge resolves, and all provenance references are valid.
2. Build a time-ordered history index by card and customer, partitioned for
  exclusive-cutoff retrieval and deterministic replay.
3. Materialize card-to-merchant, card-to-device, card-to-category,
  card-to-country, customer-to-device, card-amount, initiator, hour-of-day,
  recent-attempt, duplicate-candidate, and retry-candidate summaries.
4. Materialize one checkpoint immediately before each of the 45 purchase
  attempts. A checkpoint contains only the facts used by that attempt's
  scenario evaluation, its evidence statuses, and links to raw graph IDs.
5. Materialize the ten ordered `SCEN0001` checkpoints and expected
  deterministic simulation results as the baseline acceptance fixture.
6. Record summary statuses distinctly: `known`, `known_but_unapproved`,
  `no_prior_relationship`, `not_applicable`, and missing required evidence.

End-of-history aggregates are retained for graph inspection. Scenario
checkpoints are the authoritative precomputed inputs for test replay, so an
attempt cannot accidentally consume records from later in the dataset.

### Policy binding in the graph

Represent policy as versioned graph data, not free text and not executable
runtime integration. Store each policy version with its status, scope,
effective interval, source instruction or simulation rationale, ontology
version, and content hash. A policy may be `unconfirmed`, `confirmed`, or
`simulation_assumption`; only a confirmed policy or an explicitly labelled
simulation assumption may be bound to a scenario fixture.

```text
(Scenario)-[:EVALUATED_UNDER]->(PolicyVersion)
(PolicyVersion)-[:HAS_RULE]->(PolicyRule)
(PolicyRule)-[:TARGETS_CONCEPT]->(Concept)
(PolicyRule)-[:READS_FIELD]->(DatasetField)
(PolicyRule)-[:USES_EVIDENCE]->(AggregateDefinition)
(ScenarioCheckpoint)-[:EVALUATES]->(PurchaseAttempt)
(ScenarioCheckpoint)-[:APPLIES_POLICY]->(PolicyVersion)
(ScenarioCheckpoint)-[:USES_EVIDENCE]->(MaterializedEvidence)
```

Policy rules store typed operators, thresholds in minor units, allowed values,
and the evidence definition they require. The binding records whether a rule is
hard, step-up-on-uncertainty, or explanatory only. A policy never derives its
meaning from untrusted merchant text, and an unconfirmed semantic draft never
produces a decision fixture.

For the current data, bind `simulation-scen0001-v1` only to `SCEN0001` and
label it `simulation_assumption`. Do not apply it to the other scenarios or
present it as a customer-confirmed mandate.

### Regeneration and invalidation

On any source-data, schema, ontology, policy, or aggregate-code change,
rebuild all downstream artifacts in dependency order. The manifest records
the source hash, row counts, snapshot ID, calculation version, ontology
version, policy versions, and generation timestamp. Consumers must reject an
artifact when these versions disagree.

Tests for this phase must prove source coverage, valid provenance, graph
referential integrity, exclusive-cutoff correctness at every checkpoint,
deterministic rebuilds, and policy-binding validity. They must also prove that
future data, untrusted text, and unconfirmed policy drafts cannot affect a
stored scenario result.

### Delivery sequence

1. Add the dataset manifest and persist the validated source graph.
2. Persist the temporal index and end-of-history aggregate views.
3. Generate and validate all 45 scenario checkpoints.
4. Persist policy, rule, concept, field, and aggregate-definition bindings.
5. Generate the `SCEN0001` policy-bound baseline fixture and replay report.
6. Review the offline artifacts and acceptance checks before considering any
  interface or live integration work.
