...
def build_reports(data_dir, build_dir, *, rebuild=False):
    ...
    required = ['source_graph.json', 'precomputed_evidence.json', 'historical_evidence_index.json', 'scenario_checkpoints.json', 'policy_bindings.json', 'dataset_manifest.json', 'knowledge_graph.sqlite', 'simulation_results.json']
    ...
    guide = ['# Generated reports', '', 'All generated reports and intermediate graph outputs are tracked in Git. Edit source files in ../kg_rootcause, then regenerate the pages.', '', 'From Knowledge_graph, regenerate everything with:', '', '```sh', '.venv/bin/python -m kg_rootcause.reports --rebuild', '```', '', '## Stored offline artifacts', '', '- `dataset_manifest.json`: hashes and versions for the source release.', '- `historical_evidence_index.json`: card-partitioned, time-ordered source events.', '- `precomputed_evidence.json`: end-of-history aggregate inspection view.', '- `scenario_checkpoints.json`: exclusive-cutoff context for every supplied purchase attempt.', '- `policy_bindings.json`: SCEN0001 simulation-policy bindings; all other scenarios are unbound.', '- `knowledge_graph.sqlite`: queryable embedded graph store containing nodes, relationships, aggregates, checkpoints, and policy bindings.', '', 'Query the persisted graph with:', '', '```sh', '.venv/bin/python -m kg_rootcause.storage.sqlite_graph summary', '.venv/bin/python -m kg_rootcause.storage.sqlite_graph neighbors --node-id Card:CA0001', '.venv/bin/python -m kg_rootcause.storage.sqlite_graph path --source-id Customer:CU0001 --target-id Merchant:ME0001', '```', '', '## Where the graph page logic lives', '', '- Data preparation: `../kg_rootcause/frontend_contract/explorer.py`', '- Graph drawing, selection, branching, reason display: `../kg_rootcause/frontend_contract/assets/explorer.js`', '- Page markup: `../kg_rootcause/frontend_contract/templates/knowledge_explorer.html`', '- Page styles: `../kg_rootcause/frontend_contract/assets/explorer.css` and `standalone.css`', '- Offline document wrapper: `../kg_rootcause/reporting/html.py`', '', 'The generated HTML embeds its data, CSS, and JavaScript. It has no external script dependency.', '', 'Open index.html for the report links. report_manifest.json maps each report to its sources.']
