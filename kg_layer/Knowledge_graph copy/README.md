...
- `precompute.artifacts`: offline source-release manifest, scenario checkpoints,
  and policy bindings. These artifacts are storage-only and do not integrate
  with a live interface or authorization service.
- `storage`: an embedded SQLite graph store with indexed node, relationship,
  aggregate, checkpoint, and policy tables. Query it through the offline CLI;
...
- `dataset_manifest.json`, `scenario_checkpoints.json`, `policy_bindings.json`
...
frontend adapter, production deployment, 45/100/1,000-request expansion, and
the future live-layer adapter remain subsequent work. The current SQLite store
is an embedded offline graph database, not a production graph-service choice.

## Query the precomputed graph

The rebuild writes `build/knowledge_graph.sqlite`. It stores the complete source
graph and its materialized evidence, checkpoint, and policy artifacts in one
queryable file:

```sh
.venv/bin/python -m kg_rootcause.storage.sqlite_graph summary
.venv/bin/python -m kg_rootcause.storage.sqlite_graph node --node-id Card:CA0001
.venv/bin/python -m kg_rootcause.storage.sqlite_graph neighbors --node-id Card:CA0001
.venv/bin/python -m kg_rootcause.storage.sqlite_graph path --source-id Customer:CU0001 --target-id Merchant:ME0001
```

All commands are offline and read-only. They do not authorize payments or
connect to `live_layer`.
